...
BOOL SMULogicManager<GuildSMU>::DoPostInit()
{
	__ENTER_FUNCTION
		...
		if(!bRet)
		{
			Log::SaveLog("./Log/ShareMemory.log","从数据库读取 GuildSMU_%d 失败",key);
		}
		if(bRet)
		{
			m_bReady = TRUE;
			Log::SaveLog("./Log/ShareMemory.log","从数据库读取 GuildSMU_%d 成功",key);
		}
...
BOOL SMULogicManager<MailSMU>::DoPostInit()
{
	__ENTER_FUNCTION
		...
		if(!bRet)
		{
			Log::SaveLog("./Log/ShareMemory.log","从数据库读取 MailSMU_%d 失败",key);
		}
		if(bRet)
		{
			m_bReady = TRUE;
			Log::SaveLog("./Log/ShareMemory.log","从数据库读取 MailSMU_%d 成功",key);
		}
...
BOOL	SMULogicManager<ItemSerialKeySMU>::DoSaveAll()
{
	...
	{
		Log::SaveLog("./Log/ShareMemory.log","保存ItemSerial失败。 Serial = %d ,ServerID = %d",Serial,ServerID);
		Assert(FALSE);
	}

	
	Log::SaveLog("./Log/ShareMemory.log", "保存ItemSerial成功。 Serial = %d ,ServerID = %d",Serial,ServerID);
...
#define SERIAL_GROWUP 10000
		...
		if(Serial>0)
		{
			...
			Serial = pSMU->GetSerial(SM_C_READ);
			Log::SaveLog("./Log/ShareMemory.log","从数据库读取 ItemSerialSMU=%d 成功!  Serial = %d",key,Serial);
		}
		else
		{
			AssertEx( FALSE, "数据库查询物品信息失败" );
			Log::SaveLog("./Log/ShareMemory.log","从数据库读取 ItemSerialSMU=%d 失败!  Serial = %d",key,Serial);
		}
		m_bReady = TRUE;
