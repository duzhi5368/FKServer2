...
__ENTER_FUNCTION
	...
	if( m_nSceneCount==0 )
	{
		Log::SaveLog( SERVER_LOGFILE, "SceneThread TID:%d 没有负责场景", cTid ) ;
		return ;
	}
	...
	_MY_TRY
	{
		...
		for( i=0; i<m_nSceneCount; i++ )
		{
			Log::SaveLog( SERVER_LOGFILE, "SceneThread负责场景%s TID:%d SID:%d.", 
				g_pSceneManager->GetSceneInfo(m_apScene[i]->SceneID())->m_szName,
				cTid, m_apScene[i]->SceneID() ) ;
		}
