...
BOOL	ShareMemAO::Create(SM_KEY key,UINT Size)
{
	__ENTER_FUNCTION
		...
		{
			Log::SaveLog(SHMEM_LOG_PATH,"Create:创建ShareMem文件映射失败：KET = %d， ErrorCode = %d",key,GetLastError());
			return FALSE;
		}
		...
		{	
			...
			m_Size	=	Size;
			Log::SaveLog(SHMEM_LOG_PATH,"Create:进行ShareMem文件映射成功：KET = %d",key);
			return TRUE;
		}
		else
		{
			Log::SaveLog(SHMEM_LOG_PATH,"Create:进行ShareMem文件映射失败：KET = %d， ErrorCode = %d",key,GetLastError());
			return FALSE;
		}
...
BOOL	ShareMemAO::Attach(SM_KEY key,UINT	Size)
{
	...
	{
		// Delete by fk
		// Log::SaveLog(SHMEM_LOG_PATH,"Attach:开启ShareMem文件映射失败：KET = %d， ErrorCode = %d",key, GetLastError());
		return FALSE;
	}
	...
	if(m_pHeader)
	{
		...
		m_Size	=	Size;
		Log::SaveLog(SHMEM_LOG_PATH,"Attach:进行ShareMem文件映射成功：KET = %d",key);
		return TRUE;
	}
	else
	{
		Log::SaveLog(SHMEM_LOG_PATH,"Attach:进行ShareMem文件映射失败：KET = %d， ErrorCode = %d",key, GetLastError());
		return FALSE;
	}
