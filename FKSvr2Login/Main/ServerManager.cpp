...
__ENTER_FUNCTION
	...
	_MY_TRY
	{
		...
		Log::SaveLog( LOGIN_LOGFILE, "连接服务器 IP:%s Port:%d 成功",
	...
	Log::SaveLog( LOGIN_LOGFILE, "连接WorldSvr成功 ServerID:%d", 
...
EXCEPTION:
	...
	Log::SaveLog( LOGIN_LOGFILE, "错误： 连接WorldSvr失败 IP:%s Port:%d ServerID:%d, 进行到第 %d 步骤",
...
__LEAVE_FUNCTION
	...
	Log::SaveLog( LOGIN_LOGFILE, "异常: ConnectWorldServer 进行到第 %d 步骤",step) ;
...
BOOL ServerManager::ConnectBillingServer( )
{
	...
	Log::SaveLog( LOGIN_LOGFILE, "连接 BillingServer 成功 ServerID:%d", 
		g_Config.m_LoginInfo.m_LoginID ) ;
...
EXCEPTION:
	...
	Log::SaveLog( LOGIN_LOGFILE, "错误：连接 BillingServer失败 IP:%s Port:%d ServerID:%d",
	...
	__LEAVE_FUNCTION
		...
		Log::SaveLog( LOGIN_LOGFILE, "异常: ConnectBillingServer") ;
